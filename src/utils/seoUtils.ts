...
export function generateAnimeJsonLd(anime: {
	...
	return {
		...
		sameAs: [`https://anireki.com/works/${anime.id}`],
	};
}
...
export function optimizeMetaDescription(text: string, maxLength: number = 155): string {
	...
	const sentenceEnds = [lastPeriod, lastDot, lastExclamation, lastQuestion].filter((pos) => pos > 0);

	if (sentenceEnds.length > 0) {
		const bestSentenceEnd = Math.max(...sentenceEnds);
		if (bestSentenceEnd > maxLength * 0.7) {
			return cleanText.slice(0, bestSentenceEnd + 1);
		}
	}
...
export async function validateImageUrl(url: string): Promise<boolean> {
	try {
		...
		if (error instanceof Error && error.name === "AbortError") {
			console.warn("Image validation timeout:", url);
		}
		return false;
	}
}
...
export function getLocalizedMetaDescription(
	...
	const descriptions: Record<SupportedLocale, Record<"home" | "history" | "watchLater" | "anime", string>> = {
		...
		en: {
			...
			anime:
				"Explore amazing Japanese anime series with detailed information, ratings, and tracking features.",
		},
		ja: {
...
export function generateMultilingualAnimeJsonLd(
	anime: {
		id: string;
		title: string;
		title_jp: string;
...
export function generateMultilingualWebsiteJsonLd(locale: SupportedLocale) {
	const content: Record<SupportedLocale, { name: string; description: string; inLanguage: string }> = {
		"zh-TW": {
			name: "Anireki",
