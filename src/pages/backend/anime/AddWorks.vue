...
<template>
	<Loading :console-show="false" :show="anirekiConsole.sendStatus" />
				...
				<div v-if="animeWorks.isLoaded" class="console-container">
					<div class="card">
						...
						<div class="card-item-block">
							<div class="card-item-2">
								...
								<SelBar
									...
									:selected="refSel"
									:selDataArray="media"
									v-model:selDataString="refMedia"
									@click="selControl('A')"
								/>
							</div>
							<div class="card-item-2 ml-10">
								...
								<SelBar
									:selID="'B'"
									:selected="refSel"
									:selDataArray="yearsArray"
									v-model:selDataString="thisYear"
									@click="selControl('B')"
								/>
							</div>
							<div class="card-item-2 ml-10">
								...
								<SelBar
									:selID="'C'"
									:selected="refSel"
									:selDataObject="seaSon"
									v-model:selDataString="thisSeason"
									@click="selControl('C')"
								/>
							</div>
						</div>
						<div class="card-item">
							...
							<Transition>
								<div v-if="showAnnictResults && animeWorks.annictWorks.length > 0" class="annict-results">
									...
									<div
										v-for="work in animeWorks.annictWorks"
										:key="work.id"
										class="annict-work"
									...
									>
										<div class="annict-work-image">
											<img
												v-if="work.images && work.images.recommended_url"
												:src="work.images.recommended_url"
												alt="作品封面"
											/>
											<img
												v-else-if="work.images && work.images.facebook && work.images.facebook.og_image_url"
										...
										<div class="annict-work-info">
											...
											<div class="annict-work-details">
												<span v-if="work.season_name && work.season_year"
													>{{ work.season_year }} {{ work.season_name }}</span
												>
												<span v-if="work.media">{{ work.media }}</span>
												<span v-if="work.released_on" class="annict-work-date"
													>放送日: {{ work.released_on }}</span
												>
											</div>
										</div>
									</div>
						...
						<div v-if="selectedAnnictWorkId" class="card-item external-links-container">
							<div class="external-links">
								<a
									:href="`https://annict.com/works/${selectedAnnictWorkId}`"
									target="_blank"
								...
								<a
									:href="`https://abema.tv/search?q=${encodeURIComponent(refTitle_jp)}`"
									target="_blank"
									class="external-link abema-link"
								>
